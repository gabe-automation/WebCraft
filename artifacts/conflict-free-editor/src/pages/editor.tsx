...
function Editor() {
  ...
  const sendMessage = useCallback((message: ClientMessage) => {
    ...
    if (socket?.readyState === WebSocket.OPEN) {
      ...
      setSaveState('Sending shared change');
      return true;
    }
  ...
  const handleNodePointerDown = (event: PointerEvent<HTMLDivElement>, node: PageNode) => {
    ...
    dragRef.current = { id: node.id, pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, nodeX: node.x, nodeY: node.y };
  ...
  const handleNodePointerMove = (event: PointerEvent<HTMLDivElement>, node: PageNode) => {
    ...
    setNodes((current) => current.map((item) => item.id === node.id ? { ...item, x, y, lastEditedBy: actorId } : item));
    sendMessage({ type: 'move', nodeId: node.id, x, y, width: node.width });
    setSaveState(connection === 'live' ? 'Broadcasting position…' : 'Offline · local position retained');
  };
